    ...
    return (
        <div className='container-fluid'>
            ...
            <div className='row'>
                ...
                <div className="bg6">
                <div className='col-12 col-md-11 mx-auto d-flex p-3 mt-3 align-items-center justify-content-between bg-dark instant1 rounded mb-3'>
                    <div className='text-start'>
                        <h4 className='text-light'>Need some help?</h4>
                        <p className='text-light'>Have a browse through our most common questions<br />
                            If you need further assistance, our <Link to='/contact' className='linked'>Support Team</Link> </p>
                    </div>
                    <div>
                        <button className='button btn' onClick={() => document.location.href = '/contact'}>Go to contact</button>
                    </div>
                </div>
                ...
                <div className="row no-wrap">
                    <div className="col-12 col-md-11 mx-auto d-flex flex-column flex-md-row">
                        <div className="col-4 p-0">
                            <div className="faqs-left bg-dark ps-3 instant1 rounded">
                        ...
                        <div className="col-12 col-md-8 p-0">
                            <div className="faqs-right " ref={faqsRef}>
                                <div className='faq bg-dark rounded instant1'>
                                    <h4 className='text-light'>getting started</h4>
                                    {faqsData.map((faq, index) => (
                                        <div key={index}>
                                            <div className={`d-flex gap-3 align-items-center my-3 ${activeFaq === index ? 'faqs-active' : ''}`} onClick={() => toggleFaq(index, faq.category)}>
                                                <i className={`bi ${activeFaq === index ? 'bi-dash-circle' : 'bi-plus-circle text-light'}`} />
                                                <h5 className='text-light'>{faq.question}</h5>
                                                <hr />
                                            </div>
                                            {activeFaq === index && (
                                ...
                                <div className='faq bg-dark instant1 rounded'>
                                <h4 className='text-light'>How it works</h4>
                                    {howitworksData.map((faq, index) => (
                                        <div key={index + faqsData.length}>
                                            <div className={`d-flex gap-3 align-items-center my-3 ${activeFaq === index + faqsData.length ? 'faqs-active' : ''}`} onClick={() => toggleFaq(index + faqsData.length, faq.category)}>
                                                <i className={`bi ${activeFaq === index + faqsData.length ? 'bi-dash-circle' : 'bi-plus-circle text-light'}`} />
                                                <h5 className='text-light'>{faq.question}</h5>
                                                <hr />
                                            </div>
